...
PARENT_DB = "SESAME"   # default DB if not passed
...
TABLE_TEST_META: Dict[str, Dict[str, Any]] = {
    "CORE.ORDERS": {
        ...
        # Enable/disable tests per table here
        "tests_to_run": [
            "BUSINESS_DATE_MATCH",
...
def _metrics_to_expr(metrics: Optional[Dict[str, Any]]) -> str:
    """
    Store metrics as a JSON string in METRICS (STRING) column.
    """
    if metrics is None:
        return "NULL"
    js = json.dumps(metrics)          # {"CNT": 1234}
    js = js.replace("'", "''")        # escape quotes
    return f"'{js}'"                  # plain string literal
...
def _test_cleaned_cols(session, parent_db, tbl_cfg, run_name, business_date_value, date_filter, use_bd, debug):
    ...
    Cast to VARCHAR to avoid collation issues with REGEXP_LIKE.
    """
    ...
    for col in clean_cols:
        ...
        cond = (f"{col_expr} IS NOT NULL AND "
                f"(REGEXP_LIKE({col_expr}, '\\\\n') OR REGEXP_LIKE({col_expr}, '\\\\t'))")
...
def run_shakedown(session: Session, meta: Dict[str, Any], business_date_value: Optional[str]):
    ...
    # Return summary with PASS/FAIL as string
    return session.sql(f"""
        SELECT
            ...
            TEST_NAME,
            CASE 
                WHEN PASS_FLAG = TRUE  THEN 'PASS'
                WHEN PASS_FLAG = FALSE THEN 'FAIL'
                ELSE NULL
            END AS PASS_FLAG,
            ERROR,
            METRICS,
...
def main(session: smowpark.Session):
