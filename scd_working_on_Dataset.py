...
def perform_scd2_operations(df_existing, df_new, current_timestamp):
    ...
    df_merged = df_existing.merge(
        df_new, on="id", how="outer", suffixes=("_existing", "_new"), indicator=True
    )

    # Identify records to update (any value changed and exists in new data)
    value_columns = ["value1", "value2"]
    update_condition = (df_merged["_merge"] == "both") & df_merged[
        [f"{col}_existing" for col in value_columns]
    ].ne(df_merged[[f"{col}_new" for col in value_columns]]).any(axis=1)
    ...
    reappearing_records = df_existing[
        (df_existing["end_date"].notna()) & (df_existing["id"].isin(df_new["id"]))
    ]

    # Step 4: Update records
    df_existing.loc[df_existing["id"].isin(records_to_update["id"]), "end_date"] = (
        records_to_update["timestamp"]
    )

    # Step 5: Mark records as deleted
    df_existing.loc[df_existing["id"].isin(records_to_delete["id"]), "end_date"] = (
        current_timestamp
    )
    ...
    for index, row in reappearing_records.iterrows():
        ...
        df_existing = pd.concat(
            [df_existing, pd.DataFrame([new_record])], ignore_index=True
        )
    ...
    updated_records_to_add = records_to_update[
        ["id"] + [f"{col}_new" for col in value_columns]
    ].copy()
    ...
    df_final = pd.concat(
        [df_existing, new_records_to_add, updated_records_to_add], ignore_index=True
    )
...
if __name__ == "__main__":
    ...
    (
        ...
        records_to_delete,
        reappearing_records,
    ) = perform_scd2_operations(df_existing, df_new, current_timestamp)
