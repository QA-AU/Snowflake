...
assert 'TLAYER' in globals() and 'TGTTABLE' in globals(), "ERROR: TLAYER/TGTTABLE not found. Run Steps 2 & 3 first."
...
""").to_pandas()["C"][0]
if views_ok < 2:
    raise SystemExit("ERROR: Expected TEMP views SRC_SLICE and TGT_SLICE. Run Steps 2 and 3 first.")
...
cmp_pairs = meta[(meta["USE_SCD2"]=="Y") & (meta["IGNORE_ROW"]!="Y")][["SRC_COL","TGT_COL"]].drop_duplicates().values.tolist()
...
print(f"  Compare on : {cmp_pairs if cmp_pairs else 'NONE (only NEW/DELETED/UNCHANGED detection)'}")
print(f"  Details    : {'ON' if SHOW_DETAILS.upper()=='Y' else 'OFF'}")

print("Part C — Building FULL OUTER JOIN between SRC_SLICE and TGT_SLICE on mapped PKs ...")
...
pk_select = ",\n    ".join([f"COALESCE(s.{src}, t.{tgt}) AS {tgt}" for src, tgt in pk_pairs])

# Build diff conditions across compare columns
if cmp_pairs:
    diff_or = " OR ".join([f"(s.{s} <> t.{t} OR (s.{s} IS NULL AND t.{t} IS NOT NULL) OR (s.{s} IS NOT NULL AND t.{t} IS NULL))"
...
if SHOW_DETAILS.upper() == "Y" and cmp_pairs:
    diffs = [f"CASE WHEN (s.{s} <> t.{t} OR (s.{s} IS NULL AND t.{t} IS NOT NULL) OR (s.{s} IS NOT NULL AND t.{t} IS NULL)) THEN '{t}' END"
             for s, t in cmp_pairs]  # report DIFF names using target column names
