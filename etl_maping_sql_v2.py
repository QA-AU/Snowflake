...
TABLE_META = {
    ...
    # NEW: optional date filter; e.g. "o.order_date between ('2024-01-01' and '2024-12-31')"
    # This will be AND-ed with both source and target filters.
    "date_filter": "o.order_date between ('2024-01-01' and '2024-12-31')",
...
def build_source_sql(meta, col):
    ...
    date_flt = (meta.get("date_filter") or "").strip()  # NEW
    ...
    if debug and where_combined: print(f"[DEBUG] where_clause -> {where_combined}")
    ...
    if where_combined: sql.append("WHERE " + where_combined)
    return "\n".join(sql) + ";"
...
def build_target_sql(meta, col):
    ...
    if debug and where_combined: print(f"[DEBUG] where_clause (target) -> {where_combined}")
...
def main(session):
    """
    Full pipeline:
      1️Prepare validation SQLs for each mapping
      2️Execute and store results/errors
      3️Return summary DataFrame for review
    """
...
# ============================================================
# Run interactively in worksheet
# ============================================================
main(session)
